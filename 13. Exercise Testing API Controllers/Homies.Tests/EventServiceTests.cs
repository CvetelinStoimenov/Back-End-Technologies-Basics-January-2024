...
namespace Homies.Tests
{
    ...
    internal class EventServiceTests
    {
        ...
        public async Task AddEventAsync_ShouldAddEvent_WhenValidEventModelAndUserId()
        {
            ...
            Assert.That(eventInTheDatabase, Is.Not.Null);
        ...
        public async Task GetAllEventsAsync_ShouldReturnAllEvents()
        {
            ...
            var firstEventInTheDatabase = await _dbContext.Events.FirstOrDefaultAsync(x => x.Name == firstEventModel.Name && x.OrganiserId == userId);
            var secondEventInTheDatabase = await _dbContext.Events.FirstOrDefaultAsync(x => x.Name == secondTestEventModel.Name && x.OrganiserId == userId);
            Assert.That(firstEventInTheDatabase.Description, Is.EqualTo(firstEventModel.Description));
            Assert.That(firstEventInTheDatabase.Start, Is.EqualTo(firstEventModel.Start));
            Assert.That(firstEventInTheDatabase.End, Is.EqualTo(firstEventModel.End));
            Assert.That(secondEventInTheDatabase.Description, Is.EqualTo(secondTestEventModel.Description));
            Assert.That(secondEventInTheDatabase.Start, Is.EqualTo(secondTestEventModel.Start));
            Assert.That(secondEventInTheDatabase.End, Is.EqualTo(secondTestEventModel.End));
        }
        ...
        public async Task GetEventForEditAsync_ShouldGetEventIfPresentInTheDb()
        {
            ...
            var eventInTheDatabase = await _dbContext.Events.FirstOrDefaultAsync(x => x.Name == firstEventModel.Name && x.OrganiserId == "nonExistingUser");


            // Act
            var result = await _eventService.GetEventForEditAsync(eventInTheDatabase.Id);
